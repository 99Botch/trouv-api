...
module.exports.register = register = async (req, res, next) => {
    /**
     * 'uniqueness' a pour but avec les identifiants envoyés via la requête et de regarder l'unicité du mail et de l'username
     * et de renvoyer une erreur si ça n'est pas le cas.
     * 
     ...
     */
    const uniqueness = await User.findOne({ $or: [{ mail: req.body.mail }, { username: req.body.username }] });
    if (uniqueness) return res.status(400).json('Error | Username or Mail already taken');
     ...
     */
    let password_rules = new RegExp("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@_#\$%\^&\*])(?=.{8,})");
    let format = password_rules.exec(req.body.password);
    if (!format) return res.status(400).json('Error | Password must contain at least one lowercase, one uppercase, one number and one special character (!@_#$%^&*)');
    let hashed_pwd = await bcrypt.hash(req.body.password, 12);
     ...
     * 
     * Mes toekn on quater information essentielles: la date d'émition et d'expiration. L'username et l'user_id, ces
    ...
    */
    try {
        Promise
            .all([await user.save(), await session.save()])
...
module.exports.login = login = async (req, res, next) => {
    try {
        ...
        */        
        const token = await Session.findOne({ user_id: user._id }, { _id: 0, token: 1 });
...
module.exports.logout = logout = async (req, res, next) => {
    try {
        ...
        const token = await Session.findOneAndRemove({ user_id: req.params.id }, { _id: 1, user_id: 1 });
