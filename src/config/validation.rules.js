...
module.exports.tokenChk = tokenChk = async function (req, res, next) {
    ...
    if (!token || !session) return res.status(403).json({ message: 'Error | No token or Session is not matching user' });
    ...
    jwt.verify(token, process.env.API_KEY, async (err, user) => {
        ...
        if (req.params.id != user._id) return res.status(403).json({ message: 'Error | Profile missmatch' });
        next();
    });
}